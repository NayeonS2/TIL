...
- 기본 선택자
  ...
  - 클래스 선택자 (마침표. 문자로 시작, 해당 클래스가 적용된 항목을 선택), 아이디 선택자 (#문자로 시작, 해당 아이디가 적용된 항목 선택, 일반적으로 하나의 문서에 1번만 사용, 여러번 사용해도 동작하지만 단일 id사용을 권장), 속성 선택자
  - ```python
    * {
      color: red;     # 전체 선택자 
    }
...
- 문서 상에서 요소의 위치 결정
- static : 모든 태그의 기본값
- relative (상대위치) - normal flow 유지, 실제위치는 그대로
- absolute (절대위치) - normal flow에서 벗어남, 다음 블록 요소가 좌측 상단으로 붙음
- fixed (고정위치)
